...
- FFmpeg binaries are not committed to Git. When source-determining inputs change on `main`, a read-only job with no persisted checkout credentials assembles and audits the exact FFmpeg source, pinned public build definition and patches, and every dependency source archive selected by the resolved Windows GPL build graph. Dependency source trees are repacked with checkout-specific VCS administration data removed and deterministic ordering, ownership, and timestamps. A separate publication job receives only the verified archive and checksum, publishes them once under the pinned engine-specific tag, and never overwrites that asset.
