...
- **FR-001** The user can select or drop one local video or a directory. Folder selection uses the standard Windows picker by default. Common settings can opt into an Explorer-view Beta picker that hosts the native Shell view through `IExplorerBrowser`, shows supported video files alongside folders with Shell navigation and thumbnails, exposes the current filesystem path as clickable breadcrumbs, and provides one action in the standard confirmation position for selecting the folder currently being viewed. The chosen Shell view remains active while navigating between directories. Shell and Beta-picker labels follow the supported locale matching the Windows UI language independently of the application's display language, while the title bar and picker-owned surfaces track the active Windows light/dark color mode. Directory discovery is non-recursive and includes supported video extensions only.
...
- **FR-003** Direct WebView playback is used for compatible sources. Matroska and FFV1 sources use a temporary H.264 proxy immediately; other sources fall back to the same proxy when direct playback fails.
...
AppSettings
  ...
  loopPlayback
  explorerIntegration
  folderPicker: standard/Explorer-view-Beta mode, last confirmed path, Shell view mode, and icon size
...
| `pick_video_folder(title, selectFolderLabel, cancelLabel, initialDirectory?, initialView, viewLabels)` | UI → Rust | Host the opt-in Explorer-view Beta picker with supported videos visible and return the confirmed path plus its Shell view mode and icon size |
...
The first-run state is an accessible file/folder drop target. Common settings use one flat left-side category navigation with one detail page at a time rather than nested or cross-cutting tabs. Export settings do not interrupt each save, and all setting changes save immediately. CSS type-ramp and control-size tokens keep English and Japanese text, buttons, fields, pane controls, and status surfaces in one Windows-scaled hierarchy; the settings title is not repeated as decorative accent text. The folder-picker setting presents Windows standard and Show video files in a vertical radio group, defaults to the former, and always shows a concise explanation plus a reusable, non-interactive outlined Beta status badge beside the latter. The opt-in Explorer-view Beta picker combines a compact Back/Forward/Up row and clickable address breadcrumbs with the localized Explorer navigation and content surface; obsolete command/detail/preview/status panes are suppressed. A single top-right View button exposes Extra large icons, Large icons, Medium icons, Small icons, List, Details, Tiles, and Content with a localized native tooltip. The active view is reapplied after every directory navigation. The last confirmed directory is restored by both picker modes; the Beta picker also restores its Shell view mode and icon size across repeated selection and application restarts. A strict Zod schema is the runtime and compile-time source for `AppSettings`; unknown, obsolete, partial, or out-of-range shapes are rejected. Valid settings are persisted in `settings.json` under Tauri's application-data directory by the official Store plugin. Windows or manual mode/accent choices are projected through CSS variables; fixed app-icon artwork is strictly achromatic. MSIX registers supported-video Open with activation and a packaged COM directory command; NSIS creates equivalent current-user video and directory registrations. The application stores and applies only the directory command's visibility state, while video Open with registration remains installed. Shell-registration changes refresh the Windows Shell cache. Keyboard focus indicators are visible, icon-only actions have accessible labels, accelerators appear in tooltips, and errors appear inline.

## 9. Preview strategy

The application first reads only the required duration, frame-rate, geometry, codec, color, and declared frame-count fields with ffprobe. When FFV1 does not declare a count, its one-frame-per-packet stream is counted without decoding; other codecs retain the exact frame-counting pass. These values are required for the frame-index contract; the application reports a typed error instead of estimating a missing value. Compatible sources are exposed through Tauri's scoped asset protocol for native WebView playback. Matroska and FFV1 sources bypass unreliable direct playback, while a direct-playback error on another source triggers the same `create_preview` path. It produces an orientation-normalized, square-pixel, maximum-1280-pixel H.264 MP4 proxy with frequent keyframes. The proxy is for interaction only; final export always reads the original.
...
- The FFmpeg build identifier, immutable dated upstream GitHub Release URL, permanent engine-specific Vidmetry binary mirror, archive and executable hashes, GPL license hash, full source commits, required configuration flags and encoders, corresponding-source asset tag, and corresponding-source archive hash are pinned in `scripts/ffmpeg-sidecars.json`. Existing files and fresh downloads are both verified. The mirror contains the exact hash-verified executables and license and links to its companion complete-source Release, so upstream retention cannot break reproducible CI.
...
## 14. Acceptance criteria for 0.4.9
