...
| `cargo test --manifest-path src-tauri\Cargo.toml` | Pass — 45 Rust unit tests, including the complete supported-video filter, Windows UI language/color-mode selection, validated Shell view-state bounds, all eight View-menu mappings, history/parent navigation separation, filesystem breadcrumb targets, and constrained-width breadcrumb layout used by the native Explorer folder picker |
