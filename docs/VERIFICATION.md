...
| `npm test` | Pass — 68 tests across 11 files |
...
| `cargo test --manifest-path src-tauri\Cargo.toml` | Pass — 42 Rust unit tests, including the complete supported-video filter, Windows UI language/color-mode selection, validated Shell view-state bounds, and all eight View-menu mappings used by the native Explorer folder picker |
...
| Native folder-picker smoke | Pass — on a Japanese Windows UI in Dark mode, the opt-in `IExplorerBrowser` Beta host rendered localized Shell navigation and content; its title bar, compact Back/Forward/Up row, vertically centered address field, and confirmation row used the same Dark mode; legacy command/detail/preview panes and the old bottom Details/Large-icons controls were absent; one top-right View button showed the localized tooltip and expanded a Dark-mode menu containing Extra large icons, Large icons, Medium icons, Small icons, List, Details, Tiles, and Content with the active view checked; folders and supported video files remained visible; and the temporary test settings were restored byte-for-byte afterward |
