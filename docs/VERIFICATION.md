...
| `cargo test --manifest-path src-tauri\Cargo.toml` | Pass — 49 Rust unit tests, including FFV1 packet-count selection, neutral export staging and explicit muxer selection, bounded sharing-lock retries against both simulated and real Windows locks, the complete supported-video filter, Windows UI language/color-mode selection, and native Explorer picker view/navigation behavior |
...
| Native MKV preview smoke | Pass — a 101-frame FFV1/Matroska source took 0.040 s for its metadata probe and 0.068 s for exact packet counting, compared with 16.336 s for the former decoded-frame pass; the release app generated a 101-frame H.264/yuv420p proxy and remained responsive, without exposing proxy implementation details in the UI |
