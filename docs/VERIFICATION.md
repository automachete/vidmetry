...
| `npm test` | Pass — 67 tests across 11 files |
...
| `cargo test --manifest-path src-tauri\Cargo.toml` | Pass — 41 Rust unit tests, including the complete supported-video filter, Windows UI language/color-mode selection, and validated Shell view-state bounds used by the native Explorer folder picker |
...
| Native folder-picker smoke | Pass — on a Japanese Windows UI in Dark mode, the `IExplorerBrowser` host rendered localized Shell navigation and thumbnails; its title bar, compact Back/Forward/Up row, vertically centered address field, and confirmation row used the same Dark mode; legacy command/detail/preview panes stayed hidden while the navigation, view, and status surfaces remained available; folders and supported MP4/MKV files were shown while an unsupported TXT file was excluded; video double-clicks stayed inside the picker; the current folder was returned from the single confirmation action; and a confirmed Details view/path reopened identically after application restart |
