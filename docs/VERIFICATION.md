...
| `npm run test:licenses` | Pass — GPL corresponding-source, MPL source, third-party notice, MSIX resource, and fail-closed Release contracts |
| `npm run check` | Pass — 0 errors, 0 warnings |
| `npm run test:assets` | Pass — SVG, generated PNG/ICO pixels, and legacy tint list |
| `npm run test:shell` | Pass — MSIX covers all 16 supported video extensions and selected directories; Tauri MSI/NSIS bundling and legacy definitions are absent |
| `npm run test:msix` | Pass — unpacked manifest, classic-app activation, all 16 file associations, packaged COM directory command, x64 PE payloads, locked FFmpeg sidecars, license/source payloads, and build-file exclusions |
...
| `cargo test --manifest-path src-tauri\Cargo.toml` | Pass — 34 Rust unit tests |
...
| Application Release architecture | Pass — public/version/source preflight and Rust license audit precede parallel source transfer and Windows verification; the isolated publisher requires one MSIX plus the corresponding-source archive and checksum, and rechecks public access immediately before publication |
| `npm run msix:build` | Pass — x64 MSIX with packaged-classic activation and File Explorer COM integration; no MSI or NSIS output |
| MSIX license payload | Pass — package contains FFmpeg license/source notice, complete dependency reports, MPL text, and all 5 MPL source archives |
...
| Persistence and cache lifecycle | Pass — strict Zod rejection of incomplete, unknown, obsolete, or out-of-range settings; packaged directory-command visibility state retained across updates; unchanged media-probe reuse, explicit post-replacement invalidation, and fresh WebView asset revisions; non-empty staged cache promotion; retained-entry-safe count/size/age pruning |
...
This artifact was generated from the verified 0.4.7 source tree with the isolated `Vidmetry.Dev` / `CN=Vidmetry Development` identity. The Partner Center identity changes the final Store-submission package hash. Build outputs are intentionally not committed.

| Artifact | Size | SHA-256 |
|---|---:|---|
| `Vidmetry_0.4.7.0_x64.msix` | 114.35 MiB | `68809867962C73FF335841A7F50D3DBF913693691D2CB4BE843CC77893F7BD3F` |
