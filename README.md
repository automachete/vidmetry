...
互換MP4のエンコーダーは自動または個別に選択できます。ハードウェアエンコーダーは起動時の軽量テストで利用可否を判定し、利用できない選択肢を無効化します。保存開始時にも利用できなければ`libx264`または`libx265`へ切り替えます。

保存時は読み込み済みの動画解析結果を再利用し、長尺動画の後半トリムでは対象区間の近くから処理します。可逆保存はFFV1のスライス並列化を自動調整します。
